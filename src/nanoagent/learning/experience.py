    ...
    def analyze_reflections(self, reflections: list[dict]) -> list[ExperiencePattern]:
        ...
        for sig, records in tool_signatures.items():
            if len(records) >= 2:
                ...
                tcs = json.loads(tc_raw) if isinstance(tc_raw, str) else tc_raw
                successes = sum(
                    1 for r in records if r.get("outcome") in ("success", "partial")
                )
                failures = sum(
                    1 for r in records if r.get("outcome") in ("failure", "error")
                )
                times = [r.get("created", 0) for r in records if r.get("created")]
                pattern = ExperiencePattern(
                    trigger_context={
                        "task_hint": sample.get("task_description", "")[:120],
                        "tool_signature": sig,
                    },
                    tool_sequence=tcs,
    ...
    def match(self, context: str, limit: int = 5) -> list[ExperiencePattern]:
        ...
        for row in cursor.fetchall():
            ...
            if (
                not context
                or not hint
                or any(w in hint.lower() for w in context.lower().split())
            ):
