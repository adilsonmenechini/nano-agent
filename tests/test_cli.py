...
class TestCliHealth:
    ...
    def test_health_connected(self, runner):
        ...
        with patch.dict("nanoagent.cli._PROVIDER_FACTORY", {"fake_provider": factory}, clear=False):
            with patch("nanoagent.cli.AgentConfig") as MockConfig:
                MockConfig.return_value.providers = {
                    "fake_provider": MagicMock(api_key="k", base_url="http://u", model="m")
                }
                result = runner.invoke(_cli, ["health", "--json"])
                assert result.exit_code == 0
    ...
    def test_health_error(self, runner):
        ...
        with patch.dict("nanoagent.cli._PROVIDER_FACTORY", {"bad_provider": factory}, clear=False):
            with patch("nanoagent.cli.AgentConfig") as MockConfig:
                MockConfig.return_value.providers = {
                    "bad_provider": MagicMock(api_key="k", base_url="http://u", model="m")
                }
                result = runner.invoke(_cli, ["health", "--json"])
                assert result.exit_code == 0
...
class TestCliRun:
    def test_run_prompts_provider_invocation(self, runner):
        ...
        fake_agent.run.return_value = ("hello", {"role": "assistant", "content": "hello"})
        with patch("nanoagent.cli._make_agent", return_value=fake_agent) as mkg, \
             patch("nanoagent.cli._build_system_prompt", return_value="sys"):
            result = runner.invoke(_cli, ["run", "--prompt", "hi", "-pr", "openai", "-pp", "/tmp"])
            assert result.exit_code == 0
    ...
    def test_run_missing_provider(self, runner):
        ...
        with patch("nanoagent.cli._make_agent", return_value=None), \
