...
def test_embedding_stored_in_db():
    ...
    store.add(target="memory", scope="global", key="test", value="Semantic memory test entry")
    cursor = store.conn.execute("SELECT embedding FROM memories WHERE key = ?", ("test",))
    row = cursor.fetchone()
...
def test_semantic_search_returns_results():
    ...
    store = SQLiteMemoryStore(db_path=db_path)
    store.add(target="memory", scope="global", key="k1", value="the team meeting was scheduled for next week")
    store.add(target="memory", scope="global", key="k2", value="grocery shopping list")
    cursor = store.conn.execute("SELECT COUNT(*) FROM memories WHERE embedding IS NOT NULL")
...
def test_importance_scoring():
    ...
    score_stale = compute_importance(access_count=5, last_access=now - 86400 * 30, now=now)
...
def test_select_top_memories():
    ...
    entries = ["short", "a bit longer text here", "the longest entry in the entire list of memories"]
    scores = [0.1, 0.5, 0.9]
