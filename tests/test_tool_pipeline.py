...
class TestToolPipeline:
    ...
    def test_linear_pipeline(self, registry):
        pipe = ToolPipeline([
            ...
            PipelineStep("b", "tool_b", {"input": "{{a.result}}"}, depends_on=["a"]),
            PipelineStep("c", "tool_c", {"input": "{{b.result}}"}, depends_on=["b"]),
        ])
        results = pipe.execute(registry)
    ...
    def test_parallel_steps(self, registry):
        pipe = ToolPipeline([
            ...
            PipelineStep("c", "tool_c", {"input": "{{a.result}}"}, depends_on=["a"]),
        ])
        results = pipe.execute(registry)
    ...
    def test_diamond_dag(self, registry):
        ...
        pipe = ToolPipeline([
            ...
            PipelineStep("b", "tool_b", {"input": "{{a.result}}"}, depends_on=["a"]),
            PipelineStep("c", "tool_c", {"input": "{{a.result}}"}, depends_on=["a"]),
            PipelineStep("d", "tool_d", {"input": "{{b.result}}|{{c.result}}"}, depends_on=["b", "c"]),
        ])
        results = pipe.execute(registry)
    ...
    def test_create_pipeline_from_dicts(self, registry):
        pipe = create_pipeline([
            ...
            {"name": "b", "tool_name": "tb", "arguments": {"input": "{{a.result}}"}, "depends_on": ["a"]},
        ])
        results = pipe.execute(registry)
