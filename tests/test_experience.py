...
def test_analyze_reflections_creates_patterns():
    ...
    reflector.reflect(turn_id="t1", task_description="list files", tool_calls=[{"name": "run_shell"}], outcome="success")
    reflector.reflect(turn_id="t2", task_description="list dirs", tool_calls=[{"name": "run_shell"}], outcome="success")
...
def test_match_returns_candidates():
    store = SQLiteMemoryStore(":memory:")
    reflector = Reflector(store)
    reflector.reflect(turn_id="t1", task_description="find python files", tool_calls=[{"name": "grep_file"}], outcome="success")
    reflector.reflect(turn_id="t2", task_description="find python code", tool_calls=[{"name": "grep_file"}], outcome="success")
    engine = ExperienceEngine(store)
